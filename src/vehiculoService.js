...
// Editar un vehículo
export const editarVehiculo = async (id, km, year, precio) => {
  try {
    // Asegúrate de que precio y year son números
    const precioNumerico = parseFloat(precio);
    const yearNumerico = parseInt(year, 10);
    ...
    return { id, km, year: yearNumerico, precio: precioNumerico };
  } catch (error) {
    throw new Error('Error al editar el vehículo: ' + error.message);
  }
};